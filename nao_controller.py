...
class Nao (Robot):
    ...
    def run(self):
        ...
        last_emotion = ""
        same_count = 0
        REACT_THRESHOLD = 15    # how many same frames needed
        COOLDOWN_STEPS = 50       # wait before next reaction
        cooldown = 0
        ...
        while self.step(self.timeStep) != -1:
           ...
           # print("Detected Emotion:", emotion)
            ...
            # ===== ONLY REACT WHEN STABLE =====
            if same_count > REACT_THRESHOLD and cooldown == 0:

                print("CONFIRMED Emotion:", emotion)

                if emotion == "happy":
                    print("Robot Message : Hello! You look happy")
                    if self.speaker is not None:
                        self.speaker.speak("Hello! You look happy")
                    self.startMotion(self.handWave)
                    self.setAllLedsColor(0x00FF00)

                elif emotion == "sad":
                    print("Robot Message : It's okay, I am here for you")
                    if self.speaker is not None:
                        self.speaker.speak("It's okay, I am here for you")
                    self.startMotion(self.wipeForhead)
                    self.setAllLedsColor(0x0000FF)

                elif emotion == "angry":
                    print("Robot Message : Please calm down")
                    if self.speaker is not None:
                        self.speaker.speak("Please calm down")
                    self.startMotion(self.backwards)
                    self.setAllLedsColor(0xFF0000)

                elif emotion == "fear":
                    print("Robot Message : Don't worry, You are safe")
                    if self.speaker is not None:
                        self.speaker.speak("Don't worry, you are safe")                    
                    self.startMotion(self.taiChi)
                    self.setAllLedsColor(0xFF00FF)
                    
                elif emotion == "neutral":
                    print("Robot Message : I'm here with you")
                    if self.speaker is not None:
                        self.speaker.speak("I'm here with you")
                    self.setAllLedsColor(0xFFFFFF)
                ...
                # start cooldown so it doesn’t spam
